...
df %>% 
  ...
  dplyr:::select(country, year, gini_disp) #selecionando as variáveis do output
...
#selecionando
df2 <- df1 %>% 
  dplyr:::select(year, country, quintile, value)


#excluindo variáveis
df2 <- df1 %>% 
  dplyr:::select(-geographical_area,
         -unit,
         -notes_ids,
         -source_id,
         -indicator)
...
#excluindo por ano 2015
 ...
 df2 %>% 
   dplyr:::select(country, year, quintile, value) %>% 
